...
class RegistrationPage(BasePage):
    ...
    def set_father_name(self, fname):
        try:
            ...
            self.logger.warning("Standard Father Name ID not found, attempting to find the unlabelled input field.")
            # Unlabelled field is usually the second input after DOB or the one with no ID/placeholder
            # In our case, we'll try to find it by index or proximity if ID fails
            inputs = self.driver.find_elements(By.TAG_NAME, "input")
            # This is a bit risky but we can try to find an input that's visible and has no ID
            for inp in inputs:
                if inp.is_displayed() and not inp.get_attribute("id") and not inp.get_attribute("placeholder"):
                    self.logger.info("Found potential unlabelled field, entering father name...")
                    self.enter_text_typewriter(inp, fname)
                    return
            raise
    ...
    def handle_modal(self, timeout=1):
        # First, handle the potential site-wide password prompt
        self.handle_restricted_access()
        
        try:
            # Very short check for sweetalert modal
            self.logger.info(f"Quick check for SweetAlert modal (timeout={timeout}s)...")
            ...
            confirm_btn.click()
            self.logger.info("Dismissed SweetAlert modal")
            import time
            time.sleep(0.5)
    ...
    def wait_for_form(self, timeout=15):
        """Wait until the registration form (name field) is visible and ready."""
        from selenium.webdriver.support.wait import WebDriverWait
        from selenium.webdriver.support import expected_conditions as EC
        self.logger.info("Waiting for registration form to be ready...")
        WebDriverWait(self.driver, timeout).until(
            EC.visibility_of_element_located(self.textbox_name_id)
        )
        self.logger.info("Registration form is ready.")
